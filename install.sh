...
if [ "$EUID" -ne 0 ]; then
    ...
    wget -O /etc/stunnel/key.pem https://raw.githubusercontent.com/Sidneymegga/RustyManager/refs/heads/main/Utils/stunnel/key.pem > /dev/null 2>&1 || error_exit "Falha ao baixar key.pem"
    wget -O /etc/stunnel/stunnel.conf https://raw.githubusercontent.com/Sidneymegga/RustyManager/refs/heads/main/Utils/stunnel/conf > /dev/null 2>&1 || error_exit "Falha ao baixar config"
